import sys, info, errors, start_modules, text_utilities
...
for counter, x in enumerate(sys.argv):
    if x.startswith("--"):
        ...
        if raw_arg.lower() == "file":
            try: err_test = sys.argv[counter + 1]
            except: errors.pup_error(errors.get_error("0001"))

            file_name = " "
            for x2 in range(counter + 1, len(sys.argv)):
                if not sys.argv[x2].startswith("--"): file_name += sys.argv[x2] + " "
                else: break
